...
var sc_code_ver="H.26.1|13.10.3" //SiteCatalyst code version|y.m.d

//Specify the Report Suite ID(s) to track here
//var s_account='hpidev,hpiglobaldev'; //use 'hpiprod,hpiglobalprod' when ready to push to prod
...
s.linkInternalFilters='javascript:,heifer.org,.projectubuntu.com,heiferstage.org,heiferdev.org';
...
function s_doPlugins(s){
  ...
  /* Automate Internal Campaign Code Extraction based on iSource parameter */
	if(!s.eVar3) s.eVar3=s.getQueryParam('isource');
	s.eVar3=s.getValOnce(s.eVar3,'s_ev3',0);

  /* Automate Social Campaign Tracking based on sid parameter */
	if(!s.eVar22) s.eVar22=s.getQueryParam('sid');
  ...
  /* Visit Number */
	s.prop15=s.getVisitNum(365);

  /* Capture Hostname */
	s.server=window.location.hostname;

  /* Page URL */
	s.prop17=document.location.href.split("?")[0];

  /* Days Since Last Visit */
	s.prop16=s.getDaysSinceLastVisit();

  /* OrderID eVar */
	if(s.purchaseID) s.eVar11=s.purchaseID;

  /* Setup LinkTracking (lid, pos) */
	s.ltMode = "auto" //manual, auto
	//s.setupLinkTrack("prop11,prop12,prop13,prop14","SC_LINKS");

  /* Previous page name */
    s.prop6=s.getPreviousValue(s.pageName,'gpv','');
    
  /* Determine Add-to-Cart Location via previous page name */
	if(s.events&&s.events.indexOf('scAdd')>-1){
		s.linkTrackVars=s.apl(s.linkTrackVars,'eVar9',',',2);
		if(s.prop6) s.eVar9=s.prop6;
	}

  /* Determine whether visitor is New or a Repeat visitor within the last 30 days */
	s.eVar10=s.getNewRepeat(30);

  /* Automate Search Keyword Variables and Events*/
	s.prop9=s.getQueryParam('q');
	if(s.prop9){
		s.eVar2=s.prop9;
		s.events=s.apl(s.events,'event1',',',2);
		if(s.prop10&&(s.prop10=='0'||s.prop10=='zero')){
			s.prop10='zero';
  ...
  /* Responsive Design */
	...
	if(typeof(RespDesign) != 'undefined' && RespDesign.toString().length > 0)
		s.prop8 = s.eVar21 = RespDesign.toLowerCase();

  /* Time Parting (Central Time) */
	var tpA = s.getTimeParting('n','-6');
	s.eVar14=tpA[1] + '|' + tpA[2];

  /* Blank out products if events isn't set so we don't inflate prodViews */
  ...
  /* Lowercase All Variables */
	...
	if(s.pageType && s.pageType=="errorpage")s.pageType="errorPage";

  /* Automate Campaign Tracking Code Extraction based on mSource parameter */
	if(!s.campaign)s.campaign=s.getQueryParam('msource').toUpperCase();
	s.campaign=s.getValOnce(s.campaign,'s_campaign',0);
	
  /* Campaign Stacking:  New s.campaign values will be appended to the stack.
    Stack stored in "s_ev17" cookie. Each value is stored for 30 days.
	Max 6 values in stack at a time. ">" Delimiter. Purchase event will clear the stack.
	Sequential duplicate values won't be appended to the stack. */
	s.eVar17=s.crossVisitParticipation(s.campaign,'s_ev17','30','6','>','purchase',0);

  /* Duplicate Site Section Clean-up */
	if(s.prop5==s.prop4&&s.prop4==s.prop3&&s.prop3==s.prop2){
		s.prop5=s.prop4=s.prop3='D=c2';}
	else if(s.prop5==s.prop4&&s.prop4==s.prop3){
		s.prop5=s.prop4='D=c3';}
	else if(s.prop5==s.prop4){
		s.prop5='D=c4';}
...
/* Automate Finding Method eVar */
  ...
  /* Remove browser plugins. Not used/needed in SC15 */
    s.plugins='';
}
...
+"if(s.ltMode&&s.ltMode!='manual'){if((o.tagName&&s._TL(o.tagName)=='"
...
+"D,W];return a;}");
s._tpDST = { //used for time parting
2013:'3/10,11/3',
2014:'3/9,11/2',
2015:'3/8,11/1',
2016:'3/13,11/6',
...
/* DO NOT CHANGE - Cookie data collection variables */
s.visitorNamespace="heiferintl"
s.trackingServerSecure='pen.heifer.org';
s.trackingServer='pasture.heifer.org';
